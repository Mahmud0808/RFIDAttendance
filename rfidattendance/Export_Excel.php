...
if (isset($_POST["To_Excel"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $timein_formatted = $row['timein'] !== '00:00:00' ? date("h:i:s A", strtotime($row['timein'])) : '---';
            $timeout_formatted = $row['timeout'] !== '00:00:00' ? date("h:i:s A", strtotime($row['timeout'])) : '---';
            $checkindate_formatted = date("d/m/Y", strtotime($row['checkindate']));
            ...
            $output .= '
                        <TR> 
                            ...
                            <TD> ' . $checkindate_formatted . '</TD>
                            <TD> ' . $timein_formatted . '</TD>
                            <TD> ' . $timeout_formatted . '</TD>
                        </TR>';
        }
        $output .= '</table>';
